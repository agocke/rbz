"""CC toolchain config for linux-x64 hosts using LLVM tools.

Used inside the dotnet cross-build container where GNU binutils (ar, nm,
strip …) are absent and only their llvm-* equivalents exist.  Registering
this toolchain with --extra_toolchains lets Bazel resolve the host CC
without creating /usr/local/bin/ar → llvm-ar symlinks.
"""
...
linux_x64_llvm_cc_toolchain_config = rule(
