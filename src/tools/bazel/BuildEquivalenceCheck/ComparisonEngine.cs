...
public static class ComparisonEngine
{
    ...
    /// <summary>
    /// Determines whether an /embed: flag refers to an auto-generated file
    /// that the .NET SDK creates and embeds into the PDB for source-level
    /// debugging.  These are build artifacts, not source inputs, and Bazel
    /// does not replicate them.
    /// </summary>
    private static bool IsAutoGeneratedEmbed(string flag)
    {
        if (!flag.StartsWith("/embed:", StringComparison.Ordinal))
            return false;

        var fileName = flag["/embed:".Length..];

        // *.AssemblyAttributes.cs — [assembly: TargetFramework("...")] etc.
        if (fileName.EndsWith(".AssemblyAttributes.cs", StringComparison.Ordinal))
            return true;

        // *.AssemblyInfo.cs — [assembly: AssemblyVersion("...")] etc.
        ...
        if (fileName.EndsWith(".InternalsVisibleTo.cs", StringComparison.Ordinal))
            return true;

        // System.SR.cs / *.SharedStrings.cs — generated resource string accessors
        if (string.Equals(fileName, "System.SR.cs", StringComparison.Ordinal)
            || fileName.EndsWith(".SharedStrings.cs", StringComparison.Ordinal))
            return true;
    ...
    private static ComparisonResult CompareManagedRecords(string name, ManagedCompilationRecord msbuild, ManagedCompilationRecord bazel)
    {
        ...
        // /sourcelink: points to a build-system-generated JSON config that maps
        // source paths to repository URLs for debugger integration.  The file is
        // always auto-generated and its contents are inherently build-system-specific.
        // Bazel does not (yet) produce sourcelink configs.
        msbuildFlags.RemoveWhere(f => f.StartsWith("/sourcelink:", StringComparison.Ordinal));
        bazelFlags.RemoveWhere(f => f.StartsWith("/sourcelink:", StringComparison.Ordinal));

        // /embed: of SDK-generated files: the .NET SDK auto-generates several
        // files and embeds them into the PDB for source-level debugging.  These
        // are not source inputs — they are build artifacts that Bazel does not
        // replicate.  Filter known auto-generated patterns from both sides.
        msbuildFlags.RemoveWhere(IsAutoGeneratedEmbed);
        bazelFlags.RemoveWhere(IsAutoGeneratedEmbed);
