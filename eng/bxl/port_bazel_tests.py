        ...
        try:
            ...
            for k, v_node in attrs_ast.items():
                ...
                if k == "env":
                    val = _emit_env(val)
                if k == "srcs":
                    # Cross-package srcs ("//pkg:file.cs") are passed through
                    # verbatim — the BXL Label resolver handles workspace-relative
                    # labels of the form "//path/to/pkg:filename". External-package
                    # labels ("@pkg//...") aren't valid as srcs here and are skipped.
                    # Verify every src actually exists in the target tree (the
                    # source BUILD.bazel may reference files that have been moved
                    # or deleted upstream since the bazel-main snapshot was taken).
                    for s in val:
                        ...
                        if s.startswith("//"):
                            ws_rel = s[2:].replace(":", "/", 1)
                            if not (workspace_root / ws_rel).exists():
                                raise SkipTarget(f"missing xpkg src: {s!r}")
                        elif s.startswith(":"):
